...
use rand::{Rng, rng};
use sliding_features::{View, pure_functions::Echo, sliding_windows::Cumulative};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("f64", |b| {
        ...
        b.iter(|| {
            let mut view = Cumulative::<f64, _>::new(Echo::new(), 1024);
            for v in vals.iter() {
                view.update(*v);
    ...
    group.bench_function("f32", |b| {
        ...
        b.iter(|| {
            let mut view = Cumulative::<f32, _>::new(Echo::new(), 1024);
            for v in vals.iter() {
                view.update(*v);
                let _ = black_box(view.last());
            }
